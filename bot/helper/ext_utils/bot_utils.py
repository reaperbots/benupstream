...
def get_readable_message():
    ...
    for download in list(download_dict.values())[STATUS_START:STATUS_LIMIT+STATUS_START]:
        ...
        if download.status() not in [MirrorStatus.STATUS_SPLITTING, MirrorStatus.STATUS_SEEDING, MirrorStatus.STATUS_PROCESSING]:
            msg += f"<blockquote><b>{progress_bar(download.progress())} | {download.status()} » {download.speed()}</b>"
            msg += f"\n<b>{download.processed_bytes()} of {download.size()} | @{source(download)}</b>"
            ...
            msg += f"<blockquote>Size: {download.size()}"
        msg += f"<b> | Past:</b> {get_readable_time(time() - download.message.date.timestamp())}"
        msg += f"\n<b>❌Abort:/stop_{download.gid()[:8]}</b></blockquote>\n\n"
    if len(msg) == 0:
        return None, None
    dl_speed = 0
    up_speed = 0
    ...
    msg += f"<b>Tasks:</b> {tasks}{bmax_task} <b>| UPTM:</b> {currentTime} <b>| Free:</b> {get_readable_file_size(disk_usage('/usr/src/app/downloads/').free)}"
    return msg, button
